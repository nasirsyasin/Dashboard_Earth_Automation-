...
def test_Login_with_email():
    if ls.login_link:
        print("Login link is available")

    else:
        print("Login link is not available")


#
# @pytest.mark.csv
# def test_login_with_email():
#     if login_obj.login_link() is True:
#         login_link_status = 'Pass'
#         log_test_result("Verify that upon tapping login link user should navigate to login screen",
#                         login_link_status)
#         if login_obj.email_input() is True:
...
#             if login_obj.password_input() is True:
...
#                 if login_obj.login_btn() is True:
